...
public class MeshGenerator : MonoBehaviour
{
	...
	void CreateWallMesh(int[,] map, float squareSize)
	{
        ...
        //    float percentY = Mathf.InverseLerp(-map.GetLength(1) / 2 * squareSize, map.GetLength(1) / 2 * squareSize, wallVertices[i].y * tileAmount);
	...
	public class SquareGrid
	{
		...
		public SquareGrid(int[,] map, float squareSize)
		{
			...
			for (int x = 0; x < nodeCountX; x++)
			{
				...
				{
					//Here the position of the generated meshes is calculated
					
					Vector3 pos = new Vector3(x * squareSize, 0, y * squareSize);
					//Vector3 pos = new Vector3(x * squareSize, 0, y * squareSize);
					//bool map[x, y] == 0 controles the way the ground is either inside or outside the walls
